...
{
    if (getCookie("TechTool_Cookie") === null)
    {
        var confirmation = confirm("by browsing this site you accept cookies.");
        if (confirmation === true)
        {
            setCookie("TechTool_Cookie", 1, 365);
        }
        else
        {
            return null;
...
function prodsetstorage(id, title, link, price, users_id_users)
{
    var p = localStorage.length + 1;
    var prod = { 'id' : id, 'title' : title, 'link' : link, 'price' : price, 'users_id_users' : users_id_users};
    var array=[];
    array.push(prod);
    localStorage.setItem('product'+ p, JSON.stringify(array));
}



function prodgetstorage(id_user) {

    var j;
    var g = localStorage.length;

    for(j=0; j < g ;j++) {

        while (JSON.parse(localStorage.getItem('product' + j)) == null) {
            j++;
            g++;
        }

        if(JSON.parse(localStorage.getItem('product' + j)) != null) {
            if (id_user == JSON.parse(localStorage.getItem('product' + j))[0]['users_id_users']) {
                ...
                butdel.id = j;
                butdel.textContent = "Cancel";
                a.appendChild(butdel);
                var br = document.createElement('br');
                document.body.appendChild(br);
                p.innerHTML += JSON.parse(localStorage.getItem('product' + j))[0]['title'];
                imgcart.src = JSON.parse(localStorage.getItem('product' + j))[0]['link'];
                td5.innerHTML += JSON.parse(localStorage.getItem('product' + j))[0]['price'];

                butdel.onclick = function (j) {
                    localStorage.removeItem('product' + j.target.id);
...
function delcart(id_user) {
    ...
    for (j = 0; j < g; j++) {
        ...
        if (id_user == JSON.parse(localStorage.getItem('product' + j))[0]['users_id_users'] ) {
            ...
            obj['Data'+ d] = JSON.parse(localStorage.getItem('product' + j))[0]['title'];
